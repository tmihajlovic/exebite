...
namespace Exebite.Sheets.PodLipom
{
    public class PodLipomReader : IRestaurantReader
    {
        ...
        #region Constructor
        /// <summary>
        /// Creates new Pod lipom sheet reader with provided logger.
        /// This can fail.
        /// </summary>
        /// <param name="logger"></param>
        public PodLipomReader(ILogger logger)
        {
            _logger = logger;

            try
            {
                _reader = new SheetReader(_logger, Configuration.PodLipomSheetID);
            }
            catch (Exception ex)
            {
                logger.LogError($"Could not instantiate Pod Lipom Sheet Reader, with error {ex.Message}");
                throw;
            }
        }
        #endregion
