...
namespace Exebite.DataAccess.Repositories
{
    public class LocationQueryRepository : ILocationQueryRepository
    {
        ...
        public LocationQueryRepository(IMealOrderingContextFactory factory, IMapper mapper)
        {
            _factory = factory;
            _mapper = mapper;
        }
        ...
        public Either<Error, PagingResult<Location>> Query(LocationQueryModel queryModel)
        {
            try
            {
                ...
                using (var context = _factory.Create())
                {
                    ...
                    {
                        query = query.Where(x => x.Name.Equals(queryModel.Name, StringComparison.InvariantCultureIgnoreCase));
                    }
